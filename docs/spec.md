...
- **ASTM-HTTP Bridge**: The ASTM-HTTP bridge is a middleware component that
  ...
  production-like communication flows: Analyzer (TCP) → Bridge (TCP:5001) →
  OpenELIS (HTTP POST at `/analyzer/astm`). For testing, direct HTTP POST to
  OpenELIS is also supported. See `research.md` Section 12 and `quickstart.md`
  Step 0 for detailed setup instructions.
