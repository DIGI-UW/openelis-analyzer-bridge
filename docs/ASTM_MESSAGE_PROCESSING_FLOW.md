...
When an analyzer sends results through the ASTM-HTTP-Bridge to OpenELIS, the message follows a multi-stage processing pipeline that includes:
...
**Location**: `src/main/java/org/openelisglobal/analyzerimport/action/AnalyzerImportController.java`
...
**Location**: `src/main/java/org/openelisglobal/analyzerimport/analyzerreaders/ASTMAnalyzerReader.java`
...
The system uses **multi-strategy identification** to determine which analyzer sent the message:
...
If an analyzer has **active field mappings** configured, the system wraps the plugin inserter with a mapping-aware wrapper:
...
- **Location**: `src/main/java/org/openelisglobal/analyzer/service/MappingAwareAnalyzerLineInserter.java`
- **Responsibilities**:
...
**Workflow**:
1. **Technician Views Pending Results**: UI displays all results in `analyzer_results` table
...
1. **Error Detection**: `MappingAwareAnalyzerLineInserter` detects unmapped fields
...
The system maintains **backward compatibility** with legacy plugin-based analyzers:
...
- **Controller**: `src/main/java/org/openelisglobal/analyzerimport/action/AnalyzerImportController.java`
- **Reader**: `src/main/java/org/openelisglobal/analyzerimport/analyzerreaders/ASTMAnalyzerReader.java`
- **Mapping Inserter**: `src/main/java/org/openelisglobal/analyzer/service/MappingAwareAnalyzerLineInserter.java`
