...
public class SerialFrameBuffer {
  ...
  private final java.util.function.Supplier<org.itech.ahb.lib.astm.communication.AstmReceiptObserver> receiptFactory;
  private final java.util.function.Consumer<String> hl7Receipt;
  private org.itech.ahb.lib.astm.communication.AstmReceiptObserver receipt;
   ...
   *
   * Standalone framing utility; production listeners must supply durable receipt callbacks.
   * @param protocol the protocol declared by the pinned analyzer profile
   */
  public SerialFrameBuffer(Protocol protocol) {
    this(protocol, () -> org.itech.ahb.lib.astm.communication.AstmReceiptObserver.NONE, message -> {});
  }

  public SerialFrameBuffer(
    Protocol protocol,
    java.util.function.Supplier<org.itech.ahb.lib.astm.communication.AstmReceiptObserver> receiptFactory,
    java.util.function.Consumer<String> hl7Receipt
  ) {
    this.receiptFactory = java.util.Objects.requireNonNull(receiptFactory);
    this.hl7Receipt = java.util.Objects.requireNonNull(hl7Receipt);
  ...
  private boolean validateAndExtractFrame() {
    ...
    if (!receivedChecksum.equalsIgnoreCase(calculatedChecksum)) {
      log.warn("ASTM: Checksum mismatch. Received: {}, Calculated: {}", receivedChecksum, calculatedChecksum);
      return false;
    }

    boolean duplicate =
      frameNumber == (expectedFrameNumber + 7) % 8 && java.util.Arrays.equals(previousFrame, frameData);
    if (duplicate) return true;
    if (frameNumber != expectedFrameNumber) return false;
    byte[] exactFrame = new byte[frameData.length + 1];
    exactFrame[0] = STX;
    System.arraycopy(frameData, 0, exactFrame, 1, frameData.length);
    receipt.frame(exactFrame); // A failed commit throws before any positive ACK or accepted content.
    previousFrame = frameData;
    expectedFrameNumber = (expectedFrameNumber + 1) % 8;
    finalFrame = terminator == ETX;
  ...
  private void processHL7Byte(byte b) {
    if (b == VT) {
      ...
      if (pos > 0 && data[pos - 1] == FS) {
        ...
        hl7Receipt.accept(message); // Durable receipt must precede MSA|AA, including unregistered sources.
        completedMessages.add(message);
  ...
  private String generateHL7ACK(String originalMessage) {
    ...
    return String.format(
      "MSH|^~\\&|BRIDGE|BRIDGE|ANALYZER|ANALYZER|%s||ACK|%s|P|2.5.1\r" + "MSA|AA|%s\r",
    ...
    if (buffer.remaining() < additionalBytes) {
      ...
      if (newCapacity <= buffer.capacity()) {
        ...
        throw new IllegalStateException(
          "Serial frame exceeds maximum capacity; retained acknowledged frames require retransmission"
        );
      }
  ...
  public int getBufferSize() {
    return buffer.position() + currentASTMMessage.length();
  }
}
