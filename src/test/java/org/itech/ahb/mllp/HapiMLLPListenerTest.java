...
class HapiMLLPListenerTest {
  ...
  class MLLPFramingTests {
    ...
    void shouldHandleValidMLLPMessage() throws Exception {
      ...
      String hl7Message =
        "MSH|^~\\&|TestApp|TestFacility|DestApp|DestFac|20260205120000||ORM^O01|12345|P|2.5.1\r" +
    ...
    void shouldHandleMultipleMessages() throws Exception {
      ...
      try (Socket socket = new Socket("localhost", testPort)) {
        ...
        for (int i = 0; i < 3; i++) {
          String msg =
            "MSH|^~\\&|App" + i + "|Fac" + i + "|DestApp|DestFac|20260205120000||ORM^O01|" + (i + 100) + "|P|2.5.1\r";
  ...
  class HapiHandlerTests {
    ...
    void shouldExtractAnalyzerIdUsingHapiTerser() throws Exception {
      ...
      assertEquals("SYSMEX-XN", analyzerId);
    }
    ...
    void shouldGenerateNAKForFailure() throws Exception {
      ...
      assertTrue(response.contains("MSA|") || response.contains("MSH|"), "Should contain HL7 response structure");
    }
  ...
  /** Covers dispatch for bursts; Hl7SavedConnectionTest proves durable receipt and actual delivery. */
  @Test
  @Timeout(30)
  void rapidMessagesFromOneAddressAllReachTheReceivingApplication() throws Exception {
    TestMessageRouter testRouter = new TestMessageRouter();
    listener = new HapiMLLPListener(testPort, "connection:test", testRouter);
    listener.start();
    for (int i = 0; i < 6; i++) {
      String message = "MSH|^~\\&|App|Fac|DestApp|DestFac|20260205120000||ORM^O01|BURST" + i + "|P|2.5.1\r";
      assertTrue(sendMLLPMessage("localhost", testPort, message).contains("MSA|AA|"));
    }
    assertEquals(6, testRouter.getMessageCount());
  }
  ...
  class MessageEnvelopeTests {
    ...
    void shouldPopulateEnvelopeWithAllFields() throws Exception {
      ...
      assertNull(envelope.getResolvedAnalyzerId(), "Saved connection identity is resolved later by MessageNormalizer");
    }
  }
