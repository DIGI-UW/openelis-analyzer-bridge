...
class AnalyzerInputControllerTest {
  ...
  private static final String SAMPLE_HL7_MESSAGE =
    "MSH|^~\\&|ANALYZER|LAB|OPENELIS|HOST|20260205120000||ORU^R01|123456|P|2.5.1\r" +
    "PID|1||12345||Doe^John||19850101|M\r" +
    "OBR|1|12345||GLU^Glucose\r" +
    "OBX|1|NM|GLU^Glucose||95|mg/dL||N||F";

  private static final String SAMPLE_CSV_MESSAGE =
    "SampleID,TestCode,Result,Units,Flags,DateTime\n" +
  ...
  @Test
  void untrustedForwardingHeadersCannotChangeTheRoutedIdentityOrPort() {
    when(mockRequest.getRemoteAddr()).thenReturn("192.0.2.20");
    when(mockRequest.getRemotePort()).thenReturn(43210);
    when(mockRequest.getHeader("X-Real-IP")).thenReturn("192.0.2.10");

    var response = controller.receiveAnalyzerMessage(
      SAMPLE_ASTM_MESSAGE,
      "application/x-astm",
      "192.0.2.10",
      "9101",
    ...
    var captured = org.mockito.ArgumentCaptor.forClass(MessageEnvelope.class);
    verify(mockNormalizer).process(captured.capture());
    assertEquals("192.0.2.20", captured.getValue().getSourceId());
    assertEquals(43210, captured.getValue().getSourcePort());
  }

  @Test
  void trustedProxyChainStopsAtTheNearestUntrustedPeer() {
    ReflectionTestUtils.setField(controller, "trustedProxies", "127.0.0.1,10.0.0.1");

    assertEquals("192.0.2.20", controller.extractSourceIp("192.0.2.10, 192.0.2.20, 10.0.0.1", mockRequest));
    assertEquals("127.0.0.1", controller.extractSourceIp("192.0.2.10, invalid-peer", mockRequest));
    assertEquals("127.0.0.1", controller.extractSourceIp("192.0.2.10, ", mockRequest));
  }
  ...
  void untrustedRealIpAndPortHeadersAreIgnoredWithoutForwardedFor() {
    ...
    assertEquals("192.0.2.20", controller.extractSourceIp(null, mockRequest));
    assertEquals(43210, controller.extractSourcePort(null, "9101", mockRequest));
  }

  @Test
  void bindsTheDocumentedTrustedProxySetting() {
    try (var context = new org.springframework.context.annotation.AnnotationConfigApplicationContext()) {
      context
        ...
        .addFirst(
          new org.springframework.core.env.MapPropertySource(
            "test-proxies",
            java.util.Map.of("bridge.http.trusted-proxies", "127.0.0.1, 10.0.0.1")
          )
        );
      ...
      assertEquals(
        "192.0.2.20",
        context.getBean(AnalyzerInputController.class).extractSourceIp("192.0.2.20, 10.0.0.1", mockRequest)
      );
    }
  }
  ...
  class ContentTypeProtocolDetectionTests {
    ...
    void shouldDetectHL7FromContentType() {
      ResponseEntity<AnalyzerInputController.InputResponse> response = controller.receiveAnalyzerMessage(
        SAMPLE_HL7_MESSAGE,
        "application/hl7-v2",
        null,
        null,
  ...
  class SourceIPExtractionTests {
    ...
    @BeforeEach
    void configureTrustedProxyChain() {
      ReflectionTestUtils.setField(controller, "trustedProxies", "127.0.0.1,10.0.0.1,172.16.0.1");
    }
  ...
  class HelperMethodTests {
    ...
    void extractSourceIpEmptyXForwardedFor() {
      assertEquals("127.0.0.1", controller.extractSourceIp("", mockRequest));
      assertEquals("127.0.0.1", controller.extractSourceIp("   ", mockRequest));
    }
    ...
    void extractSourcePortProxied() {
      assertEquals(54321, controller.extractSourcePort("192.168.1.10", "54321", mockRequest));
    }
